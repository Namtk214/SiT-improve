...
def load_model(ckpt_path=None, model_size="XL"):
    ...
    For convenience, we also tolerate a few older shapes when loading:
      ...
      - Flat checkpoints that include aux-only heads like "feature_head" or
        "common_spatial_projector": drop those keys.
    """
    ...
    if ckpt_path is not None and os.path.exists(ckpt_path):
        ...
        if raw is not None:
            ...
            if isinstance(raw, collections.abc.Mapping) and (
                "feature_head" in raw or "common_spatial_projector" in raw
            ):
                raw = {
                    k: v
                    for k, v in raw.items()
                    if k not in {"feature_head", "common_spatial_projector"}
                }
            params = raw
